...
public class CenterLoginUtils {
    ...
    private static void getAccountInfo(final String token, final LoginCenterCallback callback) {
        ImbHttpClient.getAccountInfo(new ImbHttpClient.Callback<LoginAccontInfo>(LoginAccontInfo.class) {
            @Override
            public void onFailure(int code, String message) {
                ImbHttpClient.setToken(null);
                callback.onFailure(code, message);
            ...
            public void onSuccess(LoginAccontInfo result) {
                LoginAccontInfo.DataBean data = result.getData();
                ...
                LoginAccontInfo.DataBean.InstanceDetailVOSBean instance = null;
                int size = instanceDetailVOS.size();
                for (int i = 0; i < size; i++) {
                    LoginAccontInfo.DataBean.InstanceDetailVOSBean instanceDetailVOSBean = instanceDetailVOS.get(i);
     ...
     * 获取只能中心传过来的登录数据
     */
    private static int parseLoginInfo(LoginAccontInfo.DataBean.InstanceDetailVOSBean result, LoginAccontInfo.DataBean data, CenterLoginResult centerLoginResult) {
     ...
     * @param callback 回调
     */
    private static void quickLogin(final String token, final LoginCenterCallback callback) {
        ImbHttpClient.getAccountInfo(new ImbHttpClient.Callback<LoginAccontInfo>(LoginAccontInfo.class) {
            @Override
            public void onFailure(int code, String message) {
                ImbHttpClient.setToken(null);
            ...
            public void onSuccess(LoginAccontInfo result) {
                ...
                int size = instanceDetailVOS.size();
                for (int i = 0; i < size; i++) {
                    LoginAccontInfo.DataBean.InstanceDetailVOSBean instanceDetailVOSBean = instanceDetailVOS.get(0);
                    if (instanceDetailVOSBean != null) {
                        String instType = instanceDetailVOSBean.getInstType();
                        if (ImbServerResponseTranslate.isCallInstanceType(instType)) {
                            //是需要的实例
