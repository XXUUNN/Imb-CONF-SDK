...
//                android.os.Process.setThreadPriority(Process.THREAD_PRIORITY_DISPLAY);
                ...
                Log.i(TAG, "loop");
                call = ImbHttpClient.getAccountInfo(new ImbHttpClient.Callback<LoginAccountInfo>(LoginAccountInfo.class, Looper.myLooper()) {
                    @Override
    ...
    public static void stop() {
        ...
        accountStateChangeListener = null;
    }
    ...
    public static void syncNow() {
        ImbHttpClient.Callback<LoginAccountInfo> callback = new ImbHttpClient.Callback<LoginAccountInfo>(LoginAccountInfo.class, Looper.myLooper()) {
            @Override
            public void onFailure(int code, String message) {
                //是否token、失效
                if (code == ImbHttpClient.RESPONSE_CODE_BUSINESS_TOKEN_ERROR) {
     ...
     * 获取只能中心传过来的登录数据
     */
    private static void parseLoginInfo(LoginAccountInfo info) {
        LoginAccountInfo.DataBean data = info.getData();
        if (data == null) {
            return;
        }
        ...
        LoginAccountInfo.DataBean.InstanceDetailVOSBean result = instanceDetailVOS.get(0);
        if (result == null) {
            return;
        }
...
//        ReservedMeetingUtils.checkReservedMeeting(notifyCount, data.getMeetingSubject(), data.getMeetingTime());
    ...
    private static volatile AccountStateChangeListener accountStateChangeListener;

    /**
     * 注意：回调在子线程
     * @param accountStateChangeListener 状态回调
     */
